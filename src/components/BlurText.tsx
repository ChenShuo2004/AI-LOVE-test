...
function buildKeyframes(from: AnimationSnapshot, steps: AnimationSnapshot[]) {
  ...
  keys.forEach((key) => {
    let previous = from[key as keyof AnimationSnapshot] ?? 0;
    keyframes[key] = [previous];

    steps.forEach((step) => {
      previous = step[key as keyof AnimationSnapshot] ?? previous;
      keyframes[key].push(previous);
    });
  });
